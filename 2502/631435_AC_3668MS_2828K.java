...
public class Main
{
	public static void main(String [] args) throws Exception
	{
		...
		int num=-2;
		int total=0;
		stops.add(new Stop(cin.nextInt(),cin.nextInt(),num++,0));
		stops.add(new Stop(cin.nextInt(),cin.nextInt(),num++,1));
		total+=2;
		while(cin.hasNextInt())
		{
			...
			{
				stops.add(new Stop(x,y,num,total));
				total++;
			}
		}
	...
	public static double f(Stop s,Stop t)
	{
		...
		x2=t.x;
		y2=t.y;
		isTheSame=(s.subway==t.subway?true:false)&&Math.abs(s.num-t.num)==1?true:false;
		if(isTheSame)
		{
...
class Stop
{
	...
	int subway;
	int num;
	public Stop(int x,int y,int subway,int num)
	{
		this.x=x;
		this.y=y;
		this.subway=subway;
