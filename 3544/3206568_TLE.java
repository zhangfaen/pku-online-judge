...
public class Main {
    static class MinimumCostFlow {
        ...
        private AugRe aug(int src, int dest) {
            ...
            if (best[dest].equals(new BigInteger("99999999999999999999999999999")))
                return null;
            ...
            AugRe augre = new AugRe(best[dest].multiply(new BigInteger(minCap + "")), minCap);
            return augre;
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n; j++) {
                cap[i][j + n] = 1;
                cost[i][j + n] = ((w[i].multiply(p[j])).subtract(d[j].multiply(w[i]).multiply(t))).negate();
            }
        }
