...
public class Main {
	...
	static class AtomicExpression implements Expression {
		private String path;
		...
		public static Expression get(String exp) {
			try {
				...
				while (operStack.size() > 1) {
					Operation oper = operStack.poll();
					if (oper.type == Token.LEFTBRAKET)
						throw new Exception();
					Expression ne = null;
					if (oper.type == Token.NOT) {
						Expression e1 = pathStack.poll();
						checkNull(e1);
						ne = new NotExpression(e1);
					} else {
						Expression e1 = pathStack.poll();
						Expression e2 = pathStack.poll();
						checkNull(e1);
						checkNull(e2);
	...
	static class Operation extends Token {
		/*
